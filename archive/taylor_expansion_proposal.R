...
# Another way to calculate Tmat2
Tmat2 <- expit(logit(M) + cij_offsets)

# Incorrect target of inference
B <- NN_inv %*% t(z_hat) %*% Tmat %*% z_hat %*% NN_inv

# Correct B by some offset
...
for (k in 1:K) {
  for (l in 1:K) {
    ...
    drastic_taylor_adjustment[k, l] <-
      c0 * (sum((1-A_tr_subs) * mean_M_subs) - sum(A_tr_subs * mean_M_subs)) / Ikl_card
  }
}
...
C
B - taylor_adjustment
B - drastic_taylor_adjustment
B - mean_taylor_adjustment
B - estimate_mean_taylor_adjustment
...
gamma_check <- seq(0.01, 0.50, length.out = 25) # What gamma to check
...
#subsss <=
plot_df2 <- plot_df2[!(plot_df2$name %in% c('avg_xi_lower', 'avg_xi_upper', 'avg_theta_lower', 'avg_theta_upper') ), ]


A_CI_gamma <- ggplot(plot_df2) +
  geom_line(aes(x = gamma, y = value, color = name), linewidth = 1.1, alpha = 0.8) +
  ...
  # ) +
  scale_color_manual(
    values = c('avg_xi' = 'cornflowerblue',
               'avg_xi_hat' = 'cadetblue',
               'avg_varphi' = 'gold3',
               'avg_theta' = 'coral2',
