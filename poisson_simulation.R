...
figure1
ggsave('final_simulations/figures/nominal_empirical_coverage_poisson.pdf',
       plot = figure1, device = 'pdf', width = 4.5, height = 3.5)
# 
# # Create plot
# # plot_df_thinning <- plot_df %>% filter(method == 'Thinning')
...
# #                       Thinning = figure1_coverage_thinning,
# #                       Ideal = checked_coverages)
# # plot_df <- plot_df %>% pivot_longer(c('Naive', 'Thinning', 'Ideal'))
# 
# # plot_df_thinning <- plot_df %>% filter(method == 'Thinning')
...
# # legend_colors <- c('Ideal' = 'black', 'Naive' = 'skyblue', Thinning = 'coral')
# # legend_colors <- c('Ideal' = 'black',
# #                    'Naive (K = 2)' = 'sienna1', 'Naive (K = 5)' = 'sienna3', 'Naive (K = 10)' = 'sienna4',
# #                    'Thinning (K = 2)' = 'steelblue1', 'Thinning (K = 5)' = 'steelblue3', 'Thinning (K = 10)' = 'steelblue4')
# legend_colors <- c('2' = 'steelblue1', '5' = 'steelblue3', '10' = 'steelblue4')
# 
# figure1 <- ggplot()
# for (K_index in 1:length(K_check)) {
...
#                                 'Naive (K = 10)', 'Thinning (K = 2)', 
#                                 'Thinning (K = 5)', 'Thinning (K = 10)'), 
#                      values = legend_colors) +
#   xlab('Nominal Coverage') + ylab('Empirical Coverage') +
#   coord_fixed() +
...
# ggplot(plot_df) +
#   geom_line(aes(x = nominal_coverage, y = coverage, color = 'Thinning'), data = plot_df_thinning) +
...
figure2a
ggsave('final_simulations/figures/conf_width_epsilon_poisson.pdf',
...
figure2b <- ggplot(plot_df) +
  ...
  theme(aspect.ratio = 1,
        ...
        legend.text = element_text(size = 15))
figure2b
ggsave('final_simulations/figures/randindex_epsilon_poisson.pdf',
